...
import subprocess
import telebot  # Только AsyncTeleBot!
import aiohttp
import websockets
import aiofiles
import shlex
...
TELEGRAM_TOKEN = "8550778943:AAEHUopADc4iqCqR7I14kczrHLVqUwYqWH0"
COMFYUI_URL = "http://127.0.0.1:8188"
COMFYUI_OUTPUT_DIR = "output/"
COMFYUI_INPUT_DIR = "input/"
COMFYUI_COMMON_PROMT = ["",""]
...
@bot.message_handler(commands=['activate_terminal'])


async def activate_terminal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Проверка админа
    if update.effective_user.id not in ADMIN_IDS:
        await update.message.reply_text("🚫 Нет прав администратора.")
        return
    
    # Правильное извлечение команды
    cmd = " ".join(context.args)  # Всё после /activate_terminal
    if not cmd:
        await update.message.reply_text(
            "📝 Пример использования:\n"
            "• `/activate_terminal ls -la`\n"
            "• `/activate_terminal df -h`\n"
            "• `/activate_terminal ps aux | grep python`",
            parse_mode='Markdown'
        )
        return
    ...
    try:
        ...
        if '|' in cmd or '&&' in cmd or '||' in cmd or ';' in cmd:
            # Сложные команды с shell
            result = await asyncio.to_thread(
                subprocess.run, cmd, shell=True,
                capture_output=True, text=True, timeout=30
            )
        else:
            # Простые команды без shell (безопаснее)
        ...
        await update.message.reply_text("⏰ **Таймаут 30 секунд** - команда выполняется слишком долго")
    except FileNotFoundError:
        await update.message.reply_text("❌ **Команда не найдена** (проверьте написание)")
    except Exception as e:
...
def main():
    ...
    app.add_handler(CommandHandler("activate_terminal", activate_terminal))
